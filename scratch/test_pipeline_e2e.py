...
from harvester.news.pdf_parser import classify_news_categories
...
async def test_full_pipeline():
    ...
    w, h = img.size
    print(f"Testing on image: {w}x{h}")
    
    # Run Tesseract image_to_data
    ...
    median_h = max(12.0, float(sorted([l["y2"] - l["y1"] for l in lines])[len(lines)//2])) if lines else 16.0
    ...
    for hl in headlines:
        ...
        articles.append({
            "headline_original": hl_text,
            "content_original": full_body,
            "bbox": [int(art_x1), int(art_y1), int(art_x2), int(art_y2)],
            "word_count": len((hl_text + " " + full_body).split())
        })
        
    for b_idx, bb in enumerate(body_blocks):
        if b_idx not in assigned_body and len(bb["text"].strip()) > 80:
            words = bb["lines"][0]["words"]
            art_hl = " ".join(words)
            art_body = bb["text"][len(art_hl):].strip() or art_hl
            articles.append({
                "headline_original": art_hl,
                "content_original": art_body,
                "bbox": [int(bb["x1"]), int(bb["y1"]), int(bb["x2"]), int(bb["y2"])],
                "word_count": len(bb["text"].split())
            })
            
    clean_articles = [a for a in articles if a["word_count"] >= 8]
    print(f"Extracted {len(clean_articles)} clean articles. Translating top 5...")
    
    for i, a in enumerate(clean_articles[:5]):
        tr_hl = await llm_translator.translate(a["headline_original"], source_lang="ml")
        tr_body = await llm_translator.translate(a["content_original"][:500], source_lang="ml")
        a["headline_english"] = tr_hl.translated_text
        a["content_english"] = tr_body.translated_text
        cat, sec = classify_news_categories(tr_hl.translated_text, tr_body.translated_text)
        a["category"] = cat
        a["secondary_categories"] = sec
        print(f"[{i+1}] BBOX: {a['bbox']} | CAT: {cat}")
        
    Path("scratch/clean_pipeline_articles.json").write_text(json.dumps(clean_articles[:5], ensure_ascii=False, indent=2), encoding="utf-8")
    print("Saved to scratch/clean_pipeline_articles.json")
