...
class CronSchedulerManager:
    ...
    def _register_default_jobs(self):
        ...
        self.scheduler.add_job(
            ...
            name="Daily Morning Multi-Language News Digest",
            replace_existing=True
        )

        # 6. Periodic Real-time Breaking News Alert Scanner (every 10 minutes)
        async def _realtime_breaking_alerts_scan():
            try:
                from harvester.news.service import news_service
                from harvester.news.alerts_service import alerts_service
                # Scan crises/disasters and breaking business stories from national feeds
                articles = await news_service.get_news_for_source(source_id="the_hindu", category="crises_disasters", limit=5)
                biz_articles = await news_service.get_news_for_source(source_id="toi", category="business", limit=5)
                all_incoming = articles + biz_articles
                alerts_service.evaluate_live_news_articles(all_incoming)
            except Exception as e:
                logger.debug(f"[Scheduler] Real-time breaking alert scanner note: {e}")

        self.scheduler.add_job(
            _realtime_breaking_alerts_scan,
            trigger=IntervalTrigger(minutes=10),
            id="realtime_breaking_alerts_scan",
            name="Real-time Breaking News Alert Scanner (Telegram Broadcast)",
            replace_existing=True
        )
