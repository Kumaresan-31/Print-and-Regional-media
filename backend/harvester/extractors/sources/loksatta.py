...
EDITION_MAP: Dict[str, Tuple[str, str]] = {
    ...
    "nashik": ("5356", "Nasik"),
    "nasik": ("5356", "Nasik"),
    "ahilyanagar": ("28139", "Ahilyanagar"),
    "sambhajinagar": ("28138", "Chhatrapati-Sambhajinagar"),
    "aurangabad": ("28138", "Chhatrapati-Sambhajinagar"),
}


class LoksattaExtractor(BaseExtractor):
    """
    Dedicated High-Resolution Extractor for Loksatta ePaper (Indian Express Group).
    Uses Readwhere manifest & pagemeta tile API with authenticated session cookies
    to stitch crisp broadsheet newspaper pages across all Marathi regional editions.
    Composites base image tiles with transparent PNG text overlays to ensure 100% complete news.
    """

    def _get_cookies(self, edition: str) -> List[Dict[str, Any]]:
        # Check edition-specific session first, then fallback to master
        cookies = session_manager.load_cookies(f"loksatta_{edition}")
        if not cookies:
            cookies = session_manager.load_cookies("loksatta")
        return cookies
    ...
    async def _resolve_issue_id(
        ...
        """
        Resolves the Readwhere numeric issueId for the given Loksatta edition and date.
        """
        # 1. Check latest edition redirect
        latest_url = f"https://epaper.loksatta.com/t/{title_id}/latest/{edition_name}"
        try:
            async with session.get(latest_url, headers=headers, allow_redirects=True) as resp:
                final_url = str(resp.url)
                m = re.search(r"epaper\.loksatta\.com/(\d+)/", final_url)
                if m:
                    issue_id = int(m.group(1))
                    clean_date = target_date.replace("-", "")
                    if target_date in final_url or clean_date in final_url:
                        return issue_id
                    return issue_id
        except Exception as e:
            logger.warning(f"Error fetching latest URL for Loksatta {edition_name}: {e}")

        # 2. Check previous issues list for historical date
        prev_url = f"https://epaper.loksatta.com/t/{title_id}/{edition_name}"
        try:
            ...
            logger.warning(f"Error checking previous issues for Loksatta {edition_name}: {e}")
    ...
    async def extract_pages(
        ...
        title_id, edition_name = EDITION_MAP.get(ed_key, ("92", "Mumbai"))
        ...
        if progress_callback:
            progress_callback(10, 100, f"Resolving Loksatta issue for {edition_name} ({target_date})...")
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            if not issue_id:
                logger.warning(f"Could not resolve issue ID for Loksatta {edition_name} on {target_date}")
                return []

            logger.info(f"Loksatta [{edition_name}]: resolved issueId={issue_id}")

            if progress_callback:
                progress_callback(25, 100, f"Fetching page metadata for {edition_name} (Issue {issue_id})...")

            # Fetch pagemeta for all pages (e.g. 1-60)
            meta_url = f"https://epaper.loksatta.com/pagemeta/get/{issue_id}/1-60"
            ...
            try:
                ...
                logger.error(f"Failed to fetch pagemeta for Loksatta: {e}")

            if not pagemeta:
                logger.warning("No pagemeta data received for Loksatta.")
                return []

            sorted_page_keys = sorted(pagemeta.keys(), key=lambda x: int(x) if x.isdigit() else 999)
            total_pages = len(sorted_page_keys)
            logger.info(f"Loksatta [{edition_name}]: found {total_pages} pages to harvest.")

            # Concurrently download and stitch each page
            ...
            async def _process_page(p_key: str, p_idx: int) -> Optional[Path]:
                ...
                height = lvl_choice.get("height", 2569)
                ...
                async with semaphore:
                    ...
                    # Download all tiles (both base jpg background and png text overlays)
                    downloaded_chunks = await asyncio.gather(*[_fetch_chunk(c) for c in chunks])

                    # 1. Paste all base background image tiles
                    base_tiles_count = 0
                    ...
                    # 2. Paste transparent png text overlays (contains all the article headlines & Marathi text)
            ...
            for fut in asyncio.as_completed(tasks):
                ...
                if progress_callback:
                    ...
                    progress_callback(pct, 100, f"Harvested and composited {done_count}/{total_pages} Loksatta broadsheet pages...")

        # Sort pages numerically
        page_image_paths.sort(key=lambda p: p.name)
        logger.info(f"Loksatta [{edition_name}]: successfully extracted {len(page_image_paths)} pages!")
